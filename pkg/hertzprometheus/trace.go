...
func genLabels(ctx *app.RequestContext) prom.Labels {
	...
	labels[labelMethod] = defaultValIfEmpty(bs.B2S(ctx.Request.Method()), unknownLabelValue)
	labels[labelStatusCode] = defaultValIfEmpty(strconv.Itoa(ctx.Response.Header.StatusCode()), unknownLabelValue)
	labels[labelPath] = defaultValIfEmpty(bs.B2S(ctx.Request.Path()), unknownLabelValue)
